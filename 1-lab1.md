...
从kernel.asm可以看出来 kernel中的entry.s 从此处初始化栈，esp位于0xf010f000。（可能因为我之前在exercise8中改过monitor.c的文件，所以和别人的 esp指向的是整个堆栈中正在被使用的部分的最低地址。在这个地址之下的更低的地址空间都是还没有被利用的堆栈空间。）
