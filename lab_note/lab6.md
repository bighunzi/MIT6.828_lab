...
**然而，网卡驱动程序将不足以使您的操作系统连接到互联网。在新的lab6代码中，我们为您提供了一个网络栈和一个网络服务器。与之前的实验一样，使用git获取此实验室的代码，合并到您自己的代码中，并探索新的net/目录中的内容，以及kern/中的新文件。**

**除了编写驱动程序，您还需要创建一个系统调用接口来访问您的驱动程序。您将实现缺失的网络服务器代码，以在网络堆栈和驱动程序之间传输数据包。您还将通过完成web服务器将所有内容捆绑在一起。使用新的web服务器，您将能够从您的文件系统提供文件。**

大部分内核设备驱动程序代码都必须从头编写。这个实验提供的指导比以前的实验少得多:没有框架文件，没有系统调用接口，许多设计决策都留给您。出于这个原因，我们建议你在开始任何单独的练习之前阅读整个作业。很多学生觉得这个实验比以前的实验更难，所以请合理地安排时间。
...
要使用更大的数据包计数进行测试，请尝试`make E1000_DEBUG=TXERR,TX NET_CFLAGS=-DTESTOUTPUT_COUNT=100 run-net_testoutput`。如果它溢出了你的发送环，仔细检查你正在正确地处理DD状态位，并且你已经告诉硬件设置了DD状态位(使用RS命令位)。
...
一旦你完成了web服务器,启动web服务器(`make run-httpd-nox`)并在浏览器输入http://host:port/index.html ,host是电脑运行QEMU的名字（host是localhost）， port是`make which-ports`使得web服务器报告的端口号。您应该看到由运行在JOS中的HTTP服务器提供的web页面。
