...
在/ boot引导。S中，我们安装了一个全局描述符表(GDT)，通过将所有段基址设置为0并限制为0xffffffff来有效地禁用段转换。因此，“选择器”没有作用，线性地址总是等于虚拟地址的偏移量。在实验室3中，我们将不得不与分割进行更多的交互，以设置特权级别，但对于内存转换，我们可以在整个JOS实验室中忽略分割，而只关注页面转换。