...
JOS将处理器的32位线性地址空间分为两部分。我们将在实验3中开始加载和运行的用户环境(进程)将控制下部(lower part)的布局和内容，而内核始终保持对上部的完全控制。分隔线是由inc/memlayout.h中的符号ULIM任意定义的，为内核保留了大约256MB的虚拟地址空间。这就解释了为什么我们需要在实验1中给内核提供如此高的链接地址:否则内核的虚拟地址空间中就没有足够的空间同时映射到下面的用户环境中。您会发现，对于本部分和后面的实验，参考inc/memlayout.h中的JOS内存布局图是很有帮助的。
